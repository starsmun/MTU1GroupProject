...
public abstract class GameEngine implements KeyListener, MouseListener, MouseMotionListener {
    ...
    public void setResizable(boolean set){
        mFrame.setResizable(set);
    };

	// Return the width of the window
	public int[] returnRTDimensions(){
        Dimension actualSize = mFrame.getContentPane().getSize();

        int extraW = actualSize.width;
        int extraH = actualSize.height;

        return new int[]{extraW, extraH};
    }
    ...
    static void drawSolidRectangle(double x, double y, double w, double h) {
		// Fill a Rectangle
    ...
    // This function draws justified text on the screen at (x,y)
    // with Font (font,size)
    public void drawText(double[] spaceBounds, String s, String font, int size, String justifyF) {
        // Draw text on the screen
        mGraphics.setFont(new Font(font, Font.PLAIN, size));
        double yPosRelative, x = 0, y = 0;
        FontMetrics fm = mGraphics.getFontMetrics();
        Rectangle2D stringBounds = fm.getStringBounds(s, mGraphics);
        ...
        if(justifyF == "Centre") {
            x = (spaceBounds[2] - stringBounds.getWidth() + spaceBounds[0]) / 2d;
        }
        if (justifyF == "Left"){
            x = paddingX + spaceBounds[0];
        }
        if(justifyF == "Right"){
    ...
    // This function draws bold justified text on the screen at (x,y)
    // with Font (font,size)
    public static void drawBoldText(double[] spaceBounds, String s, String font, int size, String justifyF) {
        // Draw text on the screen
        mGraphics.setFont(new Font(font, Font.BOLD, size));
        double yPosRelative, x = 0, y = 0;
        FontMetrics fm = mGraphics.getFontMetrics();
